...
export default function Hero() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const ctx = gsap.context(() => {
        ...
        // 🔥 ILMU RAHASIA STACKING: Pinning dengan start bottom bottom!
        ScrollTrigger.create({
          trigger: sectionRef.current,
          start: "bottom bottom", // Baru ngunci layar setelah bagian paling bawah Hero tersentuh
          end: () => "+=" + window.innerHeight,
          pin: true,
          pinSpacing: false, // Membiarkan elemen About menyodok dari bawah
          onUpdate: (self) => {
            const p = self.progress;
            if (heroContentRef.current) {
              gsap.set(heroContentRef.current, {
                scale: 1 - p * 0.1, // Mengecil saat ditimpa
  ...
  return (
    // Wrapper untuk mencegah error DOM dari NextJS
    <div ref={wrapperRef} className="relative w-full">
      <section
        id="hero"
        ref={sectionRef}
        className="relative z-10 w-full min-h-screen bg-[#0C0512] overflow-hidden flex flex-col"
      >
        {/* Content Ref ini yang akan di-scale & opacity oleh GSAP saat stacking */}
        <div
          ref={heroContentRef}
          className="w-full flex-1 flex flex-col min-h-screen origin-top"
        >
          ...
          <div className="max-w-[1320px] w-full mx-auto px-6 md:px-10 lg:px-16 flex-1 flex flex-col justify-center">
            ...
            <div ref={statsRef} className="pb-32 mt-auto relative z-10 w-full">
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex-col items-center gap-2 text-gray-600 z-10 hidden md:flex">
