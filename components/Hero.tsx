...
export default function Hero() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.to(greetingRef.current, { opacity: 1, x: 0, duration: 0.8 }, 0.2)
        ...
        .to(chars, { y: "0%", opacity: 1, stagger: 0.035, duration: 0.9 }, 0.5)
        ...
        .to(
          imgRef.current,
          { opacity: 1, scale: 1, rotateY: 0, duration: 1.4, ease: "expo.out" },
          0.4,
        );
  ...
  useEffect(() => {
    ...
    if ((window as any).__lenisReady) {
      ...
      window.addEventListener("lenis-ready", startInit, { once: true });
    }

    return () => {
      window.removeEventListener("lenis-ready", startInit);
      st?.kill();
    };
  }, []);
