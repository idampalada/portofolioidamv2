...
// Karantina Efek Ketik agar tidak me-render ulang seluruh halaman
...
export default function Hero() {
  ...
  // 🔥 ILMU RAHASIA DEWA: Framer Motion Native Scroll Tracking (0% Lag)
  const { scrollY } = useScroll();
  const scaleDown = useTransform(scrollY, [0, 800], [1, 0.85]);
  const opacityFade = useTransform(scrollY, [0, 800], [1, 0.2]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const ctx = gsap.context(() => {
        ...
        // 🔥 PERBAIKAN: Fungsi GSAP Stacking (Pinning & Scrub) Dihapus Total!
        // Kita murni mengandalkan CSS Sticky + Framer Motion.
      }, containerRef);
  ...
  return (
    // 🔥 PERBAIKAN: Parent utamanya kita beri "sticky top-0 h-screen"
    <div
      ref={containerRef}
      id="hero"
      className="sticky top-0 z-10 bg-[#0C0512] h-screen w-full overflow-hidden"
    >
      {/* 🔥 PERBAIKAN: Bungkus dengan motion.div agar efek mengecil & memudar jalan otomatis */}
      <motion.div
      ...
      >
        <section
          ref={sectionRef}
          className="relative overflow-hidden h-full flex flex-col w-full"
        >
          <div className="relative w-full flex-1 flex flex-col h-full">
            ...
            <div className="max-w-[1320px] w-full mx-auto px-6 md:px-10 lg:px-16 flex-1 flex flex-col justify-center h-full">
                ...
                <div className="col-span-12 lg:col-span-6 flex justify-center lg:justify-end z-10 mt-16 lg:mt-0 pb-12">
                  <div
                    ...
                    onMouseMove={(e) => {
                      ...
                      imgX.set(
                        (e.clientX - (rect.left + rect.width / 2)) * 0.06,
                      );
                      imgY.set(
                        (e.clientY - (rect.top + rect.height / 2)) * 0.06,
                      );
                    }}
                    onMouseLeave={() => {
                      imgX.set(0);
